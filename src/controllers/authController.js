...
export const register = async (req, res) => {
    try {
        const { firstName, lastName, email, phonenumber, password, role, bio, username, dob, gender, bio, username, dob, gender } = req.body;
        ...
        const user = await User.create({ name, firstName, lastName, email, phonenumber, password: hashed, role, bio, username, dob, gender, bio, username, dob, gender });
        ...
        res.status(201).json({
            message: "Registered",
            user: { id: user._id, name: user.name, email: user.email, phonenumber: user.phonenumber, role: user.role, gender: user.gender, dob: user.dob, bio: user.bio, username: user.username, gender: user.gender, dob: user.dob, bio: user.bio, username: user.username },
            accessToken
        });
    } catch (err) {
  ...
  <div style="max-width: 500px; margin: auto; background: white; border-radius: 24px; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.1);">
    ...
    <div style="background: #1e1b4b; padding: 30px; text-align: center;">
      ...
      <p style="margin-top: 20px; font-size: 10px; color: #475569;">
        © ${new Date().getFullYear()} ${process.env.APP_NAME} Inc. 123 Tech Avenue, San Francisco, CA.
      </p>
    </div>
  </div>
</div>
